...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  // Capture and sanitize form data from POST request
  $cardType = trim($_POST['cardType']);
  $cardNumber = trim($_POST['cardNumber']);
  $cardName = trim($_POST['cardName']);
  $expiryDate = trim($_POST['expiryDate']) . '-01'; // Add day "01" to the month-year format
  $cw = trim($_POST['cw']);
  $selectedPackage = trim($_POST['selectedPackage']);
  ...
  } elseif (!preg_match('/^[a-zA-Z\s]+$/', $cardName)) { // Validate card name strictly
    $paymentStatus = "Card name can only contain letters and spaces, and cannot include numbers or special characters.";
  } else {
